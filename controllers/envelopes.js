...
exports.getEnvelopes = async (req, res) => {
  ...
  try {
		const envelopes = await pool.query(query);
...
exports.getEnvelopeById = async (req, res) => {
  ...
  try {
		const envelope = await pool.query(quey, [id]);
...
exports.addEnvelope = async (req, res) => {
  ...
  try {
		const newEnvelope = await pool.query(query, [title, budget]);
		res.status(201).send({
			status: 'Success',
...
exports.updateEnvelope = async (req, res) => {
  ...
  try {
    const updatedEnvelope = await pool.query(query, [title, budget, id]);
...
exports.deleteEnvelope = async (req, res) => {
  ...
  try {
		const record = await pool.query(envelopesQuery, [id]);
		...
		await pool.query(deleteEnvQuery, [id]);
    res.sendStatus(204);
  } catch (err) {
    return res.status(500).send({
			error: err.message
...
exports.addEnvelopeTransaction = async (req, res) => {
  ...
  try {
		...
		const envelope = await pool.query(envelopeQuery, [id])
		if (envelope.rowCount < 1) {
      return res.status(404).send({
        message: "No envelope information found",
			});
		};
		const newTransaction = await pool.query(transactionQuery, [title, amount, date, id]);
		await pool.query(updateEnvQuery, [amount, id]);
